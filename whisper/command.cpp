...
int process_general_transcription(struct whisper_context * ctx, audio_async & audio, const whisper_params & params) {
    ...
    std::string k_prompt = "Hey Tars, can we talk?";
...

        {
            ...
            if (::vad_simple(pcmf32_cur, WHISPER_SAMPLE_RATE, 1000, params.vad_thold, params.freq_thold, params.print_energy)) {
                ...
                if (!have_prompt) {
                    ...
                    curlpp::Easy request;
                    request.setOpt(curlpp::options::Url(std::string("http://localhost:8080")));
                    std::string query = txt;
                    ...
                    std::string emotion = response.substr(1, response.find("]") - 1);

                    std::cout << emotion << std::endl;

                    curlpp::Easy request1;
                    request1.setOpt(curlpp::options::Url(std::string("http://localhost:8081")));
                    if (emotion == "dismissive") {
                      request1.setOpt(new curlpp::options::PostFields("1"));
                    } else if (emotion == "question") {
                      request1.setOpt(new curlpp::options::PostFields("4"));
                    } else if (emotion == "sad") {
                      request1.setOpt(new curlpp::options::PostFields("5"));
                    } else if (emotion == "happy") {
                      request1.setOpt(new curlpp::options::PostFields("6"));
                    } else if (emotion == "smiley") {
                      request1.setOpt(new curlpp::options::PostFields("7"));
                    }

                    request1.perform();

                    piper << response.substr(response.find("]") + 1);
                    piper << "\" | piper --model ./en_US-ryan-high.onnx --output-raw | aplay -r 22050 -f S16_LE -t raw -";

                    std::string piper_cmd = piper.str();
                    std::cout << piper_cmd << std::endl;
                    std::system(piper_cmd.c_str());

                    request1.setOpt(new curlpp::options::PostFields("0"));
                    request1.perform();
